const input = require("fs")
  ...
  .replace(/bags/g, "")
  .replace(/bag/g, "") // Removing the mentions of bags since they're useless
  .split("\n")
...
function find(map, key) {
  if (map.has(key)) {
    if (map.get(key).find((el) => el == "shiny gold")) {
      ...
      return Array.from(map.get(key)).reduce(
        (acc, el) => acc || find(map, el),
        false
      );
    }
  }
...
const total = Array.from(theMap.keys())
  .map((key) => find(theMap, key))
  .reduce((acc, el) => {
    if (el === true) {
      return acc + 1;
    } else {
...
function findBags(map, key) {
  if (map.has(key)) {
    ...
    const toto = Array.from(map.get(key))
      .map((el) => {
        if (el.color == "other") {
          return el.num;
        } else {
